...
describe("formatAnalysisTerminal — sqlActivity section (Task 7)", () => {
	test("renders sqlActivity section when present", async () => {
		...
		expect(out).toContain(
			"Activity: 11945ms · AL execution: 7023ms (overlapping measures",
		);
	});
	...
	test("omits overlap line when only one duration field present", async () => {
		const result = await analyzeProfile(
			`${FIXTURES}/sampling-minimal.alcpuprofile`,
		);
		result.sqlActivity = {
			measuredSqlCount: 10,
			measuredSqlDurationMs: 5,
			sampledAttributedCostUs: 2000,
		};
		const out = formatAnalysisTerminal(result);
		expect(out).toContain("SQL Activity");
		expect(out).not.toContain("overlapping measures");
	});
	...
	test("caps statement lines at 5", async () => {
		const result = await analyzeProfile(
			`${FIXTURES}/sampling-minimal.alcpuprofile`,
		);
		const statements = Array.from({ length: 8 }, (_, i) => ({
			...SAMPLE_SQL_EVIDENCE.statements[0],
			text: `SELECT * FROM Table${i}`,
		}));
		result.patterns[0].sqlEvidence = { ...SAMPLE_SQL_EVIDENCE, statements };
		const out = formatAnalysisTerminal(result);
		for (let i = 0; i < 5; i++) {
			expect(out).toContain(`Table${i}`);
		}
		expect(out).not.toContain("Table5");
		expect(out).not.toContain("Table7");
	});
...
// ---------------------------------------------------------------------------
// Fixture-dependent SQL evidence tests (gitignored batch-recorded fixture,
// real BC captures — local-only by design, see test/core/analyzer.test.ts).
// ---------------------------------------------------------------------------

describe("formatAnalysisTerminal — sqlActivity/sqlEvidence with real batch-recorded fixture", () => {
