...
import {
	...
	extractSnippet,
	readSourceLines,
} from "../../src/source/snippets.js";

const fixtureFile = resolve(
	import.meta.dir,
	"../fixtures/source/CodeUnit50100.al",
);
...
describe("extractSnippet", () => {
	...
	it("should include line numbers", async () => {
		const snippet = await extractSnippet(fixtureFile, 3, 5, {
			lineNumbers: true,
		});
	...
	it("should handle context lines around a range", async () => {
		const snippet = await extractSnippet(fixtureFile, 10, 10, {
			contextLines: 2,
		});
