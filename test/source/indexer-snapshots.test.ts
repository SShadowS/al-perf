...
describe("Indexer output snapshots", () => {
	...
	describe("CodeUnit50100", () => {
		...
		it("ProcessRecords: loop with record ops", async () => {
			...
			// Variables
			const salesLine = proc.features.variables.find(
				(v) => v.name === "SalesLine",
			);
	...
	describe("CodeUnit50400", () => {
		it("should detect temporary record variable", async () => {
			const r = (await indexALFile(
				resolve(fixturesDir, "CodeUnit50400.al"),
				fixturesDir,
			))!;
			const proc = r.procedures.find((p) => p.name === "ProcessWithTempTable")!;
			const tempVar = proc.features.variables.find(
				(v) => v.name === "TempBuffer",
			);
		...
		it("should detect non-temporary record variable", async () => {
			...
			const realVar = proc.features.variables.find(
				(v) => v.name === "SalesLine",
			);
	...
	describe("CodeUnit50200", () => {
		it("should have 10 procedures, 0 triggers", async () => {
			const r = (await indexALFile(
				resolve(fixturesDir, "CodeUnit50200.al"),
				fixturesDir,
			))!;
			expect(r.procedures).toHaveLength(10);
	...
	describe("CodeUnit50700", () => {
		...
		it("GoodSetLoadFields: allFieldArguments on SetLoadFields", async () => {
			...
			const proc = r.procedures.find((p) => p.name === "GoodSetLoadFields")!;
			const op = proc.features.recordOps.find(
				(o) => o.type === "SetLoadFields",
			)!;
	...
	describe("Table50100", () => {
		it("should find 2 triggers and 2 procedures", async () => {
			...
			expect(r.objectId).toBe(50100);
			expect(r.triggers.map((t) => t.name).sort()).toEqual([
				"OnInsert",
				"OnModify",
			]);
			expect(r.procedures.map((p) => p.name).sort()).toEqual([
	...
	describe("buildSourceIndex", () => {
		it("should index all 32 fixture files", async () => {
			const index = await buildSourceIndex(fixturesDir);
			expect(index.files).toHaveLength(32);
