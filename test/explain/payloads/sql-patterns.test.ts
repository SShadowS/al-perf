...
import { extractSqlPatterns } from "../../../src/explain/payloads/sql-patterns.js";
...
describe("extractSqlPatterns", () => {
	...
	test("extracts table from SELECT with quoted name", () => {
		const nodes = [
			makeNode(
				'SELECT "No_","Description" FROM "Sales Line" WHERE "Document Type"=@p1',
				10,
				500,
			),
		];
	...
	test("strips GUID suffix from table names", () => {
		const nodes = [
			makeNode(
				'SELECT TOP 1 "Entry No_" FROM dbo."Item Ledger Entry$437dbf0e-84ff-417a-965d-ed2bb9650972" WHERE "Item No_"=@p1',
				5,
				300,
			),
		];
	...
	test("handles dbo. prefix", () => {
		const nodes = [
			makeNode(
				'SELECT "No_" FROM dbo."G/L Entry" WHERE "Entry No_">@p1',
				8,
				400,
			),
		];
	...
	test("handles INSERT INTO", () => {
		const nodes = [
			makeNode(
				'INSERT INTO "Change Log Entry" ("Entry No_","Date and Time") VALUES (@p1,@p2)',
				3,
				150,
			),
		];
	...
	test("handles UPDATE", () => {
		const nodes = [
			makeNode(
				'UPDATE "Sales Header" SET "Status"=@p1 WHERE "No_"=@p2',
				2,
				100,
			),
		];
	...
	test("handles bracket-quoted table names", () => {
		const nodes = [
			makeNode(
				"SELECT [No_] FROM [Sales Line] WHERE [Document Type]=@p1",
				4,
				200,
			),
		];
	...
	test("caps at 15 table groups", () => {
		...
		for (let i = 0; i < 20; i++) {
			nodes.push(
				makeNode(`SELECT "No_" FROM "Table ${i}" WHERE 1=1`, 20 - i, 100),
			);
		}
		const result = extractSqlPatterns(nodes);
	...
	test("handles MERGE statement", () => {
		const nodes = [makeNode('MERGE "Sales Line" USING @source ON 1=1', 2, 100)];
		const result = extractSqlPatterns(nodes);
		expect(result).toHaveLength(1);
		expect(result[0].table).toBe("Sales Line");
	});
