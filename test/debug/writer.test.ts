...
describe("writeCaptureToDisk", () => {
	...
	test("writes correct folder structure for developer-debug capture", async () => {
		...
		const meta = JSON.parse(
			await readFile(resolve(folder, "meta.json"), "utf-8"),
		);
		...
		const profileBytes = new Uint8Array(
			await readFile(resolve(folder, "profile.alcpuprofile")),
		);
		expect(profileBytes).toEqual(new Uint8Array([1, 2, 3, 4]));

		// Check analysis-result.json
		const analysisResult = JSON.parse(
			await readFile(resolve(folder, "analysis-result.json"), "utf-8"),
		);
		...
		expect(
			await readFile(resolve(explainDir, "system-prompt.txt"), "utf-8"),
		).toBe("explain system prompt");
		expect(
			JSON.parse(
				await readFile(resolve(explainDir, "user-payload.json"), "utf-8"),
			),
		).toEqual({ query: "explain payload" });
		expect(
			JSON.parse(
				await readFile(resolve(explainDir, "raw-response.json"), "utf-8"),
			),
		).toEqual({ result: "explain response" });
		expect(
			await readFile(resolve(explainDir, "parsed-output.txt"), "utf-8"),
		).toBe("explain parsed output");
	});
	...
	test("source zip is written when present", async () => {
		...
		const zipBytes = new Uint8Array(
			await readFile(resolve(folder, "source.zip")),
		);
	...
	test("deep subfolder is written when deep capture is present", async () => {
		...
		expect(await readFile(resolve(deepDir, "system-prompt.txt"), "utf-8")).toBe(
			"deep system prompt",
		);
		// parsedOutput is an object, so it should be JSON-serialized
		const parsedFindings = JSON.parse(
			await readFile(resolve(deepDir, "parsed-findings.json"), "utf-8"),
		);
	...
	test("batch profiles are written to profiles/ subfolder with manifest", async () => {
		...
		expect(profileFiles.sort()).toEqual([
			"profile1.alcpuprofile",
			"profile2.alcpuprofile",
		]);

		const p1 = new Uint8Array(
			await readFile(resolve(profilesDir, "profile1.alcpuprofile")),
		);
		expect(p1).toEqual(new Uint8Array([10, 20]));
		const p2 = new Uint8Array(
			await readFile(resolve(profilesDir, "profile2.alcpuprofile")),
		);
		expect(p2).toEqual(new Uint8Array([30, 40]));

		// Manifest at folder root
		const manifest = JSON.parse(
			await readFile(resolve(folder, "manifest.json"), "utf-8"),
		);
	...
	test("batch-explain subfolder is written when present", async () => {
		...
		expect(
			await readFile(resolve(batchDir, "parsed-output.txt"), "utf-8"),
		).toBe("batch parsed output");
	});
