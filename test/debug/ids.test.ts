...
describe("debug ID management", () => {
	...
	test("continues from highest existing folder ID", async () => {
		await mkdir(resolve(TEST_DIR, "003_2026-03-08T19-30-45"), {
			recursive: true,
		});
		await mkdir(resolve(TEST_DIR, "007_2026-03-08T20-00-00"), {
			recursive: true,
		});
		await initIdCounter(TEST_DIR);
	...
	test("ignores non-matching folder names", async () => {
		...
		await mkdir(resolve(TEST_DIR, "005_2026-03-08T19-30-45"), {
			recursive: true,
		});
		await initIdCounter(TEST_DIR);
