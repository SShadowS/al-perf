...
- **Source-only** (7): nested-loops, unfiltered-findset, event-subscriber-with-loop-ops, event-subscriber-with-loops, dangerous-call-in-loop, external-call-in-loop, unindexed-filter
  ...
  - `unfiltered-findset` counts `SetView` as filtering (it is how a caller-supplied filter string reaches the record) but **not** `SetCurrentKey`, which only picks the sort order. Like the loop detectors and `missing-setloadfields`, it gates on `isKnownNonRecordOp`: `FindSet`/`FindFirst`/`FindLast` are matched by method NAME, and `RecordRef` has all three — its filters live on `FieldRef`, so "add SetRange" is the wrong API. That guard fails OPEN on an unresolved receiver, so an implicit `Rec` and object-level globals still report.
  ...
  - **Known limitation:** `external-call-in-loop`'s declared-type gate only resolves a member's OWN local/parameter variables (`extractVariables` reads a single `var_section`, not object-level globals) — an `HttpClient` declared as an object-level global and reused across procedures is invisible to this detector (type gate fails closed, not just unrefined). Deliberately deferred; pinned by a negative test in `test/source/source-only-patterns.test.ts`.
