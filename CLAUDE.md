...
- **Source-only** (7): nested-loops, unfiltered-findset, event-subscriber-with-loop-ops, event-subscriber-with-loops, dangerous-call-in-loop, external-call-in-loop, unindexed-filter
  ...
  - `incomplete-setloadfields` cross-checks each accessed name against the RESOLVED table's field list, and the answer is three-way, not two. `Rec.SomeName` with no parentheses is a field read OR a paren-less call to a table procedure, and the parser cannot tell them apart — `Email.HasMoreDocuments` is `internal procedure HasMoreDocuments(): Boolean`, and was reported **critical** as a forgotten field. A CONFIRMED field — one present in the merged field list — is **critical** even on a fragment, because a field that is there IS a field whether or not the root was indexed. The NEGATIVE claim ("this name is not a field, therefore it is a paren-less method call, therefore skip it") is the half that needs `rootSeen`: on a fragment an absent name proves nothing, so the finding stands and hedges to **warning** with wording that names the missing root. A table that is absent entirely, or `ambiguous`, is also **warning**. FlowFields and FlowFilters are never reported at all — `SetLoadFields` does not accept them, so the suggested code would not compile; that guard alone removed 9 of the 15,436-file corpus's 83 criticals, every one a `CalcFields(Amount)`-then-read the code already handled correctly. Only the ROOT's first key counts as the always-loaded primary key, since an extension key is a secondary key by BC definition.
