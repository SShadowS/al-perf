...
**Savings estimates are measured or labelled — never silently guessed.** `what-if.ts` turns a pattern's impact into a concrete microsecond `estimatedSavings` figure by multiplying by a fixed fraction. Every fraction was originally written from judgement, and all three that have since been MEASURED were wrong in both directions: `modify-in-loop` claimed 60% and buys 7%, `dangerous-call-in-loop` claimed 90% and buys 59%, `calcfields-in-loop` claimed 80% and buys 96% (`private/matrix-probe`, `private/whatif-probe`, BC 28). The file's old note that these were "conservative" was therefore false — do not restore that framing. A model is measurable only when its suggested fix is a concrete edit (hoist the `CalcFields`, hoist the `Commit`, replace the loop with `ModifyAll`); the seven that suggest a direction instead ("reduce call frequency", "restructure to reduce per-iteration database calls") have no second variant to time, keep a rough figure for ranking, and open their explanation with `Rough estimate (not measured)`. A test asserts every model does exactly one of the two, so a new model must pick a side.
