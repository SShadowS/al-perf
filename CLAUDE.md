...
- **Source-only** (7): nested-loops, unfiltered-findset, event-subscriber-with-loop-ops, event-subscriber-with-loops, dangerous-call-in-loop, external-call-in-loop, unindexed-filter
  ...
  - **Known limitation:** `external-call-in-loop`'s declared-type gate only resolves a member's OWN local/parameter variables (`extractVariables` reads a member's parameters and its own `var_section`, not object-level globals) — an `HttpClient` declared as an object-level global and reused across procedures is invisible to this detector (type gate fails closed, not just unrefined). Deliberately deferred; pinned by a negative test in `test/source/source-only-patterns.test.ts`.
