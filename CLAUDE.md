...
**Savings estimates are measured or labelled — never silently guessed.** `what-if.ts` turns a pattern's impact into a concrete microsecond `estimatedSavings` figure by multiplying by a fixed fraction. Every fraction was originally written from judgement, and all FOUR that have since been MEASURED were wrong — two over, two under: `modify-in-loop` claimed 60% and buys 7%, `dangerous-call-in-loop` claimed 90% and buys 59%, `calcfields-in-loop` claimed 80% and buys 96%, `record-op-in-loop` claimed 70% and buys 95% (`private/matrix-probe`, `private/whatif-probe`, BC 28). The file's old note that these were "conservative" was therefore false — do not restore that framing. A model is measurable only when its suggested fix is a concrete EDIT, and making a suggestion concrete is what makes its number checkable: `record-op-in-loop` became measurable only once "restructure to reduce database calls" was replaced by the actual edit (read the looked-up table once into a temporary record before the loop) — carrying the condition that makes it safe, since preloading an unbounded table trades a slow loop for an out-of-memory error. Five of the remaining six are profile-only (`single-method-dominance`, `high-hit-count`, `repeated-siblings`, `recursive-call`, `event-chain`): with no source they know a method name, a time and a hit count, so they cannot name an edit they cannot see — making those concrete needs `--source` correlation, not better wording. The sixth, `external-call-in-loop`, names its edit but costs network latency this container cannot produce. All six keep a rough figure for ranking and open with `Rough estimate (not measured)`; a test asserts every model does exactly one of the two, so a new model must pick a side. Note also that `dangerous-call-in-loop` and `external-call-in-loop` hardcode `impact: 0` (they fire without a profile), so `annotateEstimatedSavings` skips any pattern with `impact <= 0` — otherwise a critical finding shipped "removes ~59% of the 0µs".
...
When `--source` is available, `annotateStaticCause` (`src/core/annotate-cause.ts`) appends to each profile-only finding's `suggestion` either the source-correlated findings sharing its routine, or — fenced — a scoped statement that none were raised: `No loop or SetLoadFields findings were raised for this routine.` The sentence stops there — an earlier draft continued "...so its cost is more likely computational than per-row I/O", an inference the fences below do not cover and that review found false on real routines (e.g. an HTTP call inside a loop, or an unfiltered `FindSet`, neither of which this path ever checks). The fences exist because the negative claim is the only output here that can be a confident falsehood: the routine must resolve on type AND id (`matchExactToSource`, never `matchAllToSource`, whose fallbacks can answer about a different object); the finding must name exactly one routine, which — measured against patterns.ts — only `single-method-dominance` and `recursive-call` always do (the other five build multi-anchor `involvedMethods`: `high-hit-count` and `repeated-siblings` always name exactly two, `event-chain` always names its root plus every subscriber beneath it, and `deep-call-stack` / `event-subscriber-hotspot` name a variable-length set), so the negative claim can in practice only ever reach those two detectors; and the wording says "loop or SetLoadFields findings" rather than "database anti-patterns" because the source-only family never runs in this path.
