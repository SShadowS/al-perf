...
export function computeDiagnostics(
	...
	for (const node of profile.allNodes) {
		...
		const isMetadata =
			(isTableNode(node) &&
				isMetadataTable(node.applicationDefinition.objectName)) ||
			isMetadataSqlNode(node.callFrame.functionName);
		if (isMetadata) {
			metadataSelfTime += node.selfTime;
		}
	}
	const coldCacheScore =
		profile.activeSelfTime > 0 ? metadataSelfTime / profile.activeSelfTime : 0;
	...
	if (profile.type === "instrumentation" && profile.totalDuration > 0) {
		wallClockGapRatio =
			(profile.totalDuration - profile.activeSelfTime) / profile.totalDuration;
	...
	for (const [table, entry] of tableMap) {
		...
		const accessedBy = Array.from(entry.callers.values()).sort(
	...
	const totalPatterns =
		patternCount.critical + patternCount.warning + patternCount.info;
