...
async function writeAiCapture(
	...
	await Promise.all([
		writeFile(
			resolve(dir, "system-prompt.txt"),
			capture.debugInfo.systemPrompt,
		),
		writeFile(
			resolve(dir, "user-payload.json"),
			JSON.stringify(capture.debugInfo.userPayload, null, 2),
		),
		writeFile(
			resolve(dir, "raw-response.json"),
			JSON.stringify(capture.debugInfo.rawResponse, null, 2),
		),
		writeFile(
...
export async function writeCaptureToDisk(
	...
	const name =
		folderName ?? `${padId(capture.id)}_${formatTimestamp(capture.timestamp)}`;
	...
	writes.push(
		writeFile(resolve(folder, "meta.json"), JSON.stringify(meta, null, 2)),
	);
	if (capture.analysisResult) {
		writes.push(
			writeFile(
				resolve(folder, "analysis-result.json"),
				JSON.stringify(capture.analysisResult, null, 2),
			),
		);
	}
	...
	if (capture.batchProfiles && capture.batchProfiles.length > 0) {
		...
		if (capture.manifestJson) {
			writes.push(
				writeFile(resolve(folder, "manifest.json"), capture.manifestJson),
			);
		}
	} else if (capture.profileData) {
		writes.push(
			writeFile(resolve(folder, "profile.alcpuprofile"), capture.profileData),
		);
	}

	if (capture.sourceZipData) {
		writes.push(
			writeFile(resolve(folder, "source.zip"), capture.sourceZipData),
		);
	}
