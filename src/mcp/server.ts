...
export function createMcpServer(options?: McpServerOptions): McpServer {
	...
	server.registerTool(
		"analyze_profile",
		{
			...
			description:
				"Full analysis of a Business Central .alcpuprofile file. Returns hotspots, detected patterns (N+1 queries, heavy loops, etc.), app/object breakdowns, and a one-liner summary. If a companion .zip with AL source files exists alongside the profile, source correlation is enabled automatically.",
			inputSchema: {
				...
				sourcePath: z
					...
					.optional()
					.describe(
						"Path to AL source directory (overrides auto-detection and server default)",
					),
				top: z
					.number()
					.int()
					...
					.max(100)
					.default(10)
					.describe("Number of top hotspots to return"),
				appFilter: z
					.string()
					.optional()
	...
	server.registerTool(
		"analyze_batch",
		{
			...
			description:
				"Analyze multiple AL CPU profiles and produce an aggregate report with recurring patterns, cumulative hotspots, and per-activity breakdown. Useful for analyzing scheduled profiler output.",
			inputSchema: {
				profilePaths: z
					.array(z.string())
					.describe("Paths to .alcpuprofile files, or a single directory path"),
				sourcePath: z
					.string()
					.optional()
					.describe(
				...
				manifestPath: z
					.string()
					.optional()
					.describe("Path to manifest.json with ProfileMetadata[]"),
				top: z
					.number()
					.int()
					.min(1)
					.max(100)
					.default(10)
					.describe("Number of top hotspots per profile"),
				appFilter: z
					.string()
					.optional()
					.describe("Comma-separated app names to focus on"),
				...
				if (resolvedPaths.length === 0) {
					return {
						content: [
							{
								type: "text" as const,
								text: "Error: No .alcpuprofile files found in the provided paths.",
							},
						],
						isError: true,
					};
	...
	server.registerTool(
		"get_hotspots",
		{
			title: "Get Hotspots",
			description:
				"Quick top-N hotspot summary from an AL CPU profile. Lighter than analyze_profile — skips pattern detection. Use this for a fast overview of where time is spent.",
			inputSchema: {
				profilePath: z.string().describe("Path to the .alcpuprofile file"),
				top: z
					.number()
	...
	server.registerTool(
		"compare_profiles",
		{
			...
			inputSchema: {
				beforePath: z
					.string()
					.describe("Path to the 'before' .alcpuprofile file"),
				afterPath: z
					.string()
					.describe("Path to the 'after' .alcpuprofile file"),
				threshold: z
					.number()
					.default(0)
	...
	server.registerTool(
		"explain_method",
		{
			...
			description:
				"Deep dive into a specific method from an AL CPU profile. Shows self time, total time, hit count, callers, and callees. Use objectId to disambiguate if multiple objects have the same method name.",
			inputSchema: {
				profilePath: z.string().describe("Path to the .alcpuprofile file"),
				...
				objectId: z
					...
					.optional()
					.describe(
						"Object ID to disambiguate when multiple objects have the same method",
					),
			},
		},
				...
				if (candidates.length === 0) {
					return {
						content: [
							{
								type: "text" as const,
								text: `Error: Method "${method}" not found in profile.`,
							},
						],
						isError: true,
					};
	...
	server.registerTool(
		"analyze_source",
		{
			...
			description:
				"Analyze AL source files for structural patterns without needing a profile. Parses .al files with tree-sitter, identifies objects, procedures, triggers, loops, and record operations. Reports structural findings like record operations inside loops (potential N+1 queries).",
			inputSchema: {
				sourcePath: z
					.string()
					.describe("Path to directory containing .al source files"),
			},
		},
		async ({ sourcePath }) => {
			try {
	...
	server.registerTool(
		"gate_check",
		{
			...
			description:
				"CI/CD quality gate — checks if a profile exceeds pattern thresholds. Returns pass/fail verdict with pattern counts. Use this to validate that a profile meets quality standards before deployment.",
			inputSchema: {
				profilePath: z.string().describe("Path to the .alcpuprofile file"),
				sourcePath: z
					.string()
					.optional()
					.describe("Path to AL source directory"),
				maxCritical: z
					.number()
					.int()
					.min(0)
					.default(0)
					.describe("Max critical patterns before failing (default: 0)"),
				maxWarning: z
					.number()
					.int()
					.min(0)
					.optional()
					.describe("Max warning patterns before failing (default: unlimited)"),
			},
		},
	...
	server.registerTool(
		"drilldown_method",
		{
			...
			description:
				"Deep dive into a specific method from an AL CPU profile. Shows how its totalTime is distributed across its own selfTime and child method contributions. Use objectId to disambiguate if multiple objects have the same method name.",
			inputSchema: {
				profilePath: z.string().describe("Path to the .alcpuprofile file"),
	...
	server.registerTool(
		"visualize_flamegraph",
		{
			...
			description:
				"Generate an interactive flamegraph SVG by sending the profile to the AL-Flamegraph service. Returns the path to the saved SVG file.",
			inputSchema: {
				profilePath: z.string().describe("Path to the .alcpuprofile file"),
				serviceUrl: z
					.string()
					.default("http://localhost:5000")
					.describe("AL-Flamegraph service URL"),
			},
		},
		async ({ profilePath, serviceUrl }) => {
			try {
				...
				if (!response.ok) {
					return {
						content: [
							{
								type: "text" as const,
								text: `Flamegraph service error: ${response.status} ${response.statusText}`,
							},
						],
						isError: true,
					};
				...
				const svgPath = profilePath.replace(
					/\.alcpuprofile$/,
					".flamegraph.svg",
				);
				...
				return {
					content: [
						{
							type: "text" as const,
							text: JSON.stringify({ svgPath, size: svg.length }),
						},
					],
				};
			} catch (err: unknown) {
				...
				return {
					content: [
						{
							type: "text" as const,
							text: `Failed to connect to flamegraph service: ${message}`,
						},
					],
					isError: true,
				};
	...
	server.registerTool(
		"history_list",
		{
			...
			description:
				"List past analysis history entries. Filter by profile path or label. Returns entries sorted newest-first.",
			inputSchema: {
				profilePath: z
					.string()
					.optional()
					.describe("Filter by profile path (substring match)"),
	...
	server.registerTool(
		"history_trend",
		{
			...
			description:
				"Compute trend deltas between the oldest and newest history entries. Shows how selfTime, healthScore, and patternCount changed over time.",
			inputSchema: {
				profilePath: z
					.string()
					.optional()
				...
				limit: z
					...
					.max(100)
					.default(10)
					.describe("Number of recent entries to consider"),
			},
		},
		async ({ profilePath, limit }) => {
			try {
						...
						content: [
							{
								...
								text: JSON.stringify({
									message:
										"Need at least 2 history entries to compute a trend.",
				...
				const trend = {
					...
					deltas: {
						totalSelfTime:
							last.metrics.totalSelfTime - first.metrics.totalSelfTime,
						healthScore: last.metrics.healthScore - first.metrics.healthScore,
						patternCount: {
							critical:
								last.metrics.patternCount.critical -
								first.metrics.patternCount.critical,
							warning:
								last.metrics.patternCount.warning -
								first.metrics.patternCount.warning,
							info:
								last.metrics.patternCount.info -
								first.metrics.patternCount.info,
						},
					},
					series: sorted.map((e) => ({
						id: e.id,
	...
	server.registerResource(
		...
		"resource://al-profiler/pattern-docs",
		{
			...
			description:
				"Reference documentation for all detected patterns (profile-only and source-correlated).",
	...
	server.registerResource(
		...
		"resource://al-profiler/last-analysis",
		{
			...
			description:
				"The most recent AnalysisResult from analyze_profile. Returns null if no analysis has been run yet.",
