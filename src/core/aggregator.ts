...
export function aggregateByMethod(
	...
	for (const entry of map.values()) {
		...
		entry.efficiencyScore =
			entry.totalTime > 0 ? entry.selfTime / entry.totalTime : 0;
	}
	...
	for (const node of profile.allNodes) {
		...
		if (
			entry.callAmplification === undefined ||
			ratio > entry.callAmplification
		) {
		...
		const sorted = Array.from(lines.entries())
			.map(([line, executionTime]) => ({
				...
				executionTimePercent:
					entry.selfTime > 0 ? (executionTime / entry.selfTime) * 100 : 0,
			}))
...
export function aggregateByObject(
	profile: ProcessedProfile,
): ObjectBreakdown[] {
