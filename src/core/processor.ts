...
export function processProfile(parsed: ParsedProfile): ProcessedProfile {
	...
	for (const node of allNodes) {
		if (isIdleNode(node)) {
			...
			node.selfTimePercent =
				activeSelfTime > 0 ? (node.selfTime / activeSelfTime) * 100 : 0;
			node.totalTimePercent =
				activeSelfTime > 0 ? (node.totalTime / activeSelfTime) * 100 : 0;
		}
	}
