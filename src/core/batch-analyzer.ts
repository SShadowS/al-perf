...
export async function analyzeBatch(
	...
	const alignedMetadata = options?.metadata
		? succeededIndices
				.map((i) => options.metadata![i])
				.filter((m): m is ProfileMetadata => m != null)
		: undefined;
...
function computeBatchMeta(
	...
	const totalDuration = results.reduce(
		(sum, r) => sum + r.meta.totalSelfTime,
		0,
	);
...
function aggregatePatterns(
	...
	return Array.from(map.values())
		.map((entry) => ({
			...
			totalProfiles,
			recurrencePercent: Math.round(
				(entry.profileIndices.size / totalProfiles) * 100,
			),
			affectedActivities: Array.from(entry.profileIndices).map((i) => {
				if (metadata?.[i]) return metadata[i].activityDescription;
...
function mergeAppBreakdowns(results: AnalysisResult[]): AppBreakdown[] {
	...
	const totalSelfTime = Array.from(map.values()).reduce(
		(sum, a) => sum + a.selfTime,
		0,
	);
...
function computeBatchSummary(
	...
	const weightedHealth =
		totalDuration > 0
			? results.reduce(
					(sum, r) => sum + r.summary.healthScore * r.meta.totalSelfTime,
					0,
				) / totalDuration
			: 0;
