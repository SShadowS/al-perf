...
export function buildTableBreakdown(
	...
	const sourceInfo = new Map<
		string,
		{ hasSetLoadFields: boolean; hasFilters: boolean }
	>();
	if (sourceIndex) {
		for (const obj of sourceIndex.objects.values()) {
			...
			for (const member of members) {
				for (const op of member.features.recordOps) {
					...
					if (op.type === "SetRange" || op.type === "SetFilter")
						info.hasFilters = true;
				}
			}
		}
	}
	...
	for (const [tableName, entry] of tableMap) {
		const operationBreakdown: TableOperationBreakdown[] = Array.from(
			entry.ops.entries(),
		)
			.map(([operation, data]) => ({
				operation,
