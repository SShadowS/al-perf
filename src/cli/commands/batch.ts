...
export function registerBatchCommand(program: Command) {
	program
		...
		.argument(
			"<paths...>",
			"Profile files or directories containing .alcpuprofile files",
		)
		.option(
			"-f, --format <format>",
			"Output format: auto|terminal|json|markdown|html",
			"auto",
		)
		...
		.option(
			"-s, --source <path>",
			"Path to AL source directory (enables source correlation)",
		)
		.option("--cache", "Cache source index for faster re-analysis")
		.option(
			"--explain",
			"Append AI-generated batch analysis summary (requires ANTHROPIC_API_KEY)",
		)
		.option(
			"--deep",
			"Enable deep AI analysis (not yet supported for batch; falls back to --explain)",
		)
		.option(
			"--model <model>",
			"Model for --explain: sonnet (default) or opus",
			"sonnet",
		)
		.option(
			"--api-key <key>",
		...
		.option(
			"--debug",
			"Save debug capture (prompts, payloads, responses) to debug/ folder",
		)
		.action(async (paths: string[], opts: any) => {
			if (opts.deep) {
				console.error(
					"Note: --deep is not yet supported for batch analysis. Using --explain instead.",
				);
				opts.explain = true;
			}
			...
			if (profilePaths.length === 0) {
				console.error(
					"Error: No .alcpuprofile files found in the provided paths.",
				);
				process.exit(1);
			}
			...
			if (sourcePath && opts.cache) {
				const cache = new SourceIndexCache(
					resolve(sourcePath, ".al-profile-cache"),
				);
			...
			if (opts.explain) {
				...
				if (model !== "sonnet" && model !== "opus") {
					console.error(
						`Error: --model must be "sonnet" or "opus", got "${model}"`,
					);
					process.exit(1);
				}
				...
				if (!apiKey) {
					console.error(
						"Warning: --explain requires an API key. Set ANTHROPIC_API_KEY or use --api-key.",
					);
				} else {
					try {
						...
						batchExplainResult = await withStatus(
							...
							() =>
								mod.explainBatchAnalysis(result, {
									apiKey,
						...
						console.error(
							`[api-cost] ${formatCallCost(batchExplainResult.cost)}`,
						);
					} catch (err: unknown) {
						const message = err instanceof Error ? err.message : String(err);
			...
			if (result.errors.length > 0) {
				for (const err of result.errors) {
					console.error(
						`Warning: Failed to analyze ${err.profilePath}: ${err.error}`,
					);
				}
			}

			process.stdout.write(
				formatBatch(result, opts.format as OutputFormat) + "\n",
			);
			...
			if (opts.debug) {
				...
				const capture: DebugCapture = {
					...
					batchProfiles,
					manifestJson: opts.manifest
						? readFileSync(resolve(opts.manifest), "utf-8")
						: undefined,
