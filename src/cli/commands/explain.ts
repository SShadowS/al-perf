	...
	.action(async (profilePath: string, methodName: string, opts) => {
		...
		if (candidates.length === 0) {
			process.stderr.write(
				`Error: Method "${methodName}" not found in profile.\n`,
			);
			process.exit(1);
		}
		...
		if (format === "json") {
			...
			lines.push(
				`  Object: ${method.objectType} ${method.objectId} (${method.objectName})`,
			);
			lines.push(`  App: ${method.appName}`);
			lines.push("");
			lines.push(chalk.bold("Profile Stats"));
			lines.push(
				`  Self Time:  ${result.profileStats.selfTime} (${result.profileStats.selfTimePercent})`,
			);
			lines.push(
				`  Total Time: ${result.profileStats.totalTime} (${result.profileStats.totalTimePercent})`,
			);
