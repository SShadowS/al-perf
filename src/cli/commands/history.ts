...
export function createHistoryCommand(): Command {
	...
	cmd
		...
		.action((opts) => {
			...
			for (const e of entries) {
				const barStr = "\u2588".repeat(
					Math.min(50, Math.round(e.metrics.healthScore / 2)),
				);
				const label = e.label ? ` [${e.label}]` : "";
				console.log(
					`  ${e.timestamp.slice(0, 10)}  ${barStr} ${e.metrics.healthScore}/100  ${formatTime(e.metrics.totalSelfTime)}${label}`,
				);
			}
			...
			console.log("");
			console.log(
				`  Trend: ${sign}${formatTime(delta)} over ${entries.length} entries`,
			);
		});
