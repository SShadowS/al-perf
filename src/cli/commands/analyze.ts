...
export function registerAnalyzeCommand(program: Command) {
	program
		...
		.option(
			"--explain",
			"Append AI-generated analysis summary (requires ANTHROPIC_API_KEY)",
		)
		.option(
			"--deep",
			"Enable deep AI analysis with structured findings (implies --explain, requires ANTHROPIC_API_KEY)",
		)
		.option(
			"--model <model>",
		...
		.option(
			"--label <label>",
			"Label for this analysis run (e.g., 'baseline', 'after-fix')",
		)
		.option(
			"--debug",
		...
		.action(async (profilePath: string, opts: any) => {
			...
			if (!sourcePath) {
				...
				if (zipPath) {
					const extracted = await withStatus(
						"Extracting companion source archive...",
			...
			const result = await withStatus("Analyzing profile...", () =>
				analyzeProfile(profilePath, {
					...
					sourceIndex,
					onProcessedProfile: opts.deep
						? (p: ProcessedProfile) => {
								processedProfile = p;
							}
						: undefined,
					onSourceIndex: opts.deep
						? (idx: SourceIndex) => {
			...
			if (opts.explain) {
				...
				if (!apiKey) {
					...
					try {
						...
						explainResult = await withStatus(
							`Generating AI explanation (Claude ${modelLabel})...`,
							() =>
			...
			if (opts.deep) {
				...
				if (!apiKey) {
					console.error(
						"Warning: --deep requires an API key. Set ANTHROPIC_API_KEY or use --api-key.",
					);
				} else if (processedProfile) {
					try {
						const model = opts.model as ExplainModel;
						const modelLabel = model === "opus" ? "Opus" : "Sonnet";
						deepResult = await withStatus(
							`Running deep AI analysis (Claude ${modelLabel})...`,
							() =>
			...
			process.stdout.write(
				formatAnalysis(result, opts.format as OutputFormat) + "\n",
			);
