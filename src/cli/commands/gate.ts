...
export function registerGateCommand(program: Command) {
	program
		...
		.option(
			"--max-critical <n>",
			"Max critical patterns before failing (default: 0)",
			"0",
		)
		.option(
			"--max-warning <n>",
			"Max warning patterns before failing (default: unlimited)",
		)
		...
		.action(async (profilePath: string, opts: any) => {
			...
			const maxWarning =
				opts.maxWarning !== undefined
					? parseInt(opts.maxWarning, 10)
					: Infinity;
			...
			const result: GateResult = {
				...
				thresholds: {
					maxCritical,
					maxWarning: maxWarning === Infinity ? null : maxWarning,
				},
				violations,
			...
			if (opts.format === "json") {
				...
				process.stdout.write(
					`${icon}: ${counts.critical} critical, ${counts.warning} warning, ${counts.info} info\n`,
				);
