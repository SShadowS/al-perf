...
function renderBatchSummary(result: BatchAnalysisResult): string {
	...
	if (result.meta.timeRange) {
		lines.push(
			`| Time Range | ${result.meta.timeRange.start} — ${result.meta.timeRange.end} |`,
		);
	}
	...
	if (result.summary.worstProfile) {
		...
		lines.push(
			`| Worst Profile | ${worst.description} (health ${worst.healthScore}/100) |`,
		);
	}
...
function renderActivityBreakdown(result: BatchAnalysisResult): string {
	...
	for (const activity of result.activityBreakdown) {
		...
		if (activity.patternCount.critical > 0)
			patternParts.push(`${activity.patternCount.critical}C`);
		if (activity.patternCount.warning > 0)
			patternParts.push(`${activity.patternCount.warning}W`);
		if (activity.patternCount.info > 0)
			patternParts.push(`${activity.patternCount.info}I`);

		const selfRefNote = activity.selfReferential
			? " \u26A0\uFE0F self-ref"
			: "";
		lines.push(
...
function renderAppBreakdown(result: BatchAnalysisResult): string {
	...
	for (const app of result.appBreakdown) {
		lines.push(
			`| ${app.appName} | ${formatTime(app.selfTime)} | ${app.selfTimePercent.toFixed(1)}% |`,
		);
	}
