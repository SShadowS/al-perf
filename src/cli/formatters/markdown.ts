...
function renderPatterns(result: AnalysisResult): string {
	...
	for (const p of result.patterns) {
		...
		if (p.sqlEvidence) {
			lines.push("");
			lines.push(renderSqlEvidenceMd(p.sqlEvidence));
		}
		lines.push("");
	}
...
function renderSqlActivity(result: AnalysisResult): string {
	...
	if (!a) return "";
	const lines: string[] = ["## SQL Activity (measured vs sampled)", ""];
	lines.push(
...
/**
 * Render per-finding SQL evidence as markdown. Two provenances, never
 * conflated: sampled profile estimates (NEVER "measured"/"exact"/"ran N
 * times") or measured threshold-gated telemetry (NEVER "sampled"). Up to 5
 * statement lines.
 */
