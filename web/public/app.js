...
// Fetch and display usage stats on the dropzone
fetch("/api/stats")
	.then((r) => r.json())
	.then((stats) => {
		const el = document.getElementById("stats");
		if (el && stats.totalAnalyses > 0) {
			el.textContent =
				stats.totalAnalyses.toLocaleString() + " profiles analyzed";
		}
	})
	.catch(() => {});
...
function renderSummary(data) {
	...
	// One-liner headline (no card wrapper — prominent)
	...
	badgesRow.className = "summary-badges";
...
function renderHotspots(data) {
	...
	function rebuildTbody() {
		...
		for (let i = 0; i < sorted.length; i++) {
			...
			if (h.sourceLocation) {
				tdObj.innerHTML =
					escapeHtml(h.objectType) +
					" " +
					h.objectId +
					'<br><span style="color:var(--text-secondary);font-size:0.85em">' +
					escapeHtml(h.sourceLocation.filePath) +
					":" +
			...
			const gapHtml =
				h.gapTime && h.gapTime > 0
					? ' <span style="color:var(--warning-color,#9F9700)">+' +
						escapeHtml(formatTime(h.gapTime)) +
						" wait</span>"
					: "";
			tdSelf.innerHTML =
...
function renderAppBreakdown(data) {
	...
	const sorted = [...data.appBreakdown].sort((a, b) => b.selfTime - a.selfTime);
...
function renderTableBreakdown(data) {
	...
	for (const t of data.tableBreakdown) {
		...
		tdTime.textContent =
			formatTime(t.totalSelfTime) +
			" (" +
			t.totalSelfTimePercent.toFixed(1) +
			"%)";
		...
		if (t.operationBreakdown && t.operationBreakdown.length > 0) {
			tdOp.textContent =
				t.operationBreakdown[0].operation +
				" (" +
				formatTime(t.operationBreakdown[0].selfTime) +
				")";
		} else {
			tdOp.textContent = "\u2014";
		}
		...
		tdSlf.style.color = t.hasSetLoadFields
		...
		tdFilt.style.color = t.hasFilters
			? "var(--success-color, #4CAF50)"
			: "var(--text-secondary)";
	...
	for (const obj of sorted) {
		...
		if (obj.methods && obj.methods.length > 0) {
			...
			for (const m of obj.methods) {
				...
				const tdTotal = document.createElement("td");
				tdTotal.textContent =
					formatTime(m.totalTime) + " (" + m.totalTimePercent.toFixed(1) + "%)";
				tr.appendChild(tdTotal);
...
function renderAiDeep(data) {
	...
	if (findings.length === 0 && narrative) {
		try {
			...
			if (findings.length === 0) {
				...
				if (findings.length > 0) {
					narrative =
						"*(AI analysis was truncated — showing salvaged findings)*";
				}
			}
		} catch {
	...
	if (hasFindings) {
		...
		for (const f of sorted) {
			...
			badge.textContent =
				severityIcon[f.severity] + " " + capitalize(f.severity);
			headerRow.appendChild(badge);
...
async function handleFiles(files) {
	const profileFiles = Array.from(files).filter((f) =>
		f.name.endsWith(".alcpuprofile"),
	);
...
async function handleSingleUpload(profileFile, sourceFile) {
	...
	try {
		const data = await fetchWithStream(
			"/api/analyze?stream=1",
			formData,
			(step, message) => {
				if (statusEl) statusEl.textContent = message;
				if (stepsEl && !seenSteps.has(step)) {
					// Mark previous step as done
					const prev = stepsEl.querySelector(".progress-step.active");
					if (prev) {
					...
					stepEl.innerHTML =
						'<span class="progress-step-dot"></span>' + escapeHtml(message);
...
function renderBatchSummary(data) {
	...
	if (data.meta.timeRange) {
		stats.push({
			label: "Time Range",
			value:
				new Date(data.meta.timeRange.start).toLocaleTimeString() +
				" - " +
				new Date(data.meta.timeRange.end).toLocaleTimeString(),
		});
	}
	stats.push({
		label: "Health",
		value: data.summary.overallHealthScore + "/100",
	});
	...
	batchBadge.style.background = "rgba(91, 143, 217, 0.12)";
	batchBadge.style.color = "var(--severity-info)";
	...
	if (data.summary.worstProfile) {
		...
		callout.className = "suggestion-box";
		callout.style.marginTop = "1.5rem";
		callout.style.borderLeftColor = "var(--severity-warning)";
		callout.innerHTML =
			'<span style="color:var(--text-secondary)">Worst profile:</span> ' +
			"<strong>" +
			escapeHtml(wp.description || wp.profilePath) +
			"</strong>" +
			' <span style="color:var(--text-secondary)">(health ' +
			wp.healthScore +
			"/100)</span>";
...
function renderActivityBreakdown(data) {
	...
	for (const activity of data.activityBreakdown) {
		...
		const activityName = activity.metadata?.activityDescription
		...
		if (activity.topHotspot) {
			...
			hotspotLine.innerHTML =
				...
				'<span class="mono">' +
				escapeHtml(activity.topHotspot.functionName) +
				"</span>" +
				" (" +
				escapeHtml(activity.topHotspot.objectName) +
				")" +
				" — " +
				activity.topHotspot.selfTimePercent.toFixed(1) +
				"% self time";
		...
		tdType.textContent = activity.metadata?.activityType || "\u2014";
		...
		if (pc.info > 0)
			parts.push(
				'<span class="severity-badge severity-info">' + pc.info + " I</span>",
			);
	...
	for (const p of sorted) {
		...
		if (p.affectedActivities && p.affectedActivities.length > 0) {
			...
			activitiesDiv.textContent =
				"Affected: " + p.affectedActivities.slice(0, 5).join(", ");
			if (p.affectedActivities.length > 5) {
				activitiesDiv.textContent +=
					" (+" + (p.affectedActivities.length - 5) + " more)";
			}
...
function renderCumulativeHotspots(data) {
	...
	const maxCumulTime = Math.max(
		...data.cumulativeHotspots.map((h) => h.cumulativeSelfTime),
	);
	...
	function rebuildTbody() {
		...
		for (let i = 0; i < sorted.length; i++) {
			...
			const barPct =
				maxCumulTime > 0 ? (h.cumulativeSelfTime / maxCumulTime) * 100 : 0;
			tdCumul.innerHTML =
			...
			tdProfiles.textContent =
				String(h.profileCount) + "/" + data.meta.profileCount;
...
function renderBatchErrors(data) {
	...
	for (const err of data.errors) {
		...
		card.innerHTML =
			"<strong>" +
			escapeHtml(err.profilePath.replace(/^.*[/\\]/, "")) +
			"</strong>" +
			'<div style="color:var(--severity-critical);margin-top:0.25rem">' +
 ...
 */
function buildBatchSidebar(_data) {
	const nav = document.getElementById("sidebar-nav");
	if (!nav) return;
	nav.innerHTML = "";
