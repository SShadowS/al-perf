...
import {
	...
	PAYLOAD_PRESETS,
	type PayloadConfig,
} from "../src/explain/deep-analyzer.js";
import {
	type ExplainModel,
	type ExplainResult,
	explainAnalysis,
} from "../src/explain/explainer.js";
import {
	PROMPT_PRESETS,
	type PromptConfig,
...
import {
	extractCompanionZip,
	findCompanionZip,
} from "../src/source/zip-extractor.js";
...
async function analyzeOne(
	...
	try {
		...
		log(
			`  Analysis: ${result.hotspots.length} hotspots, ${result.patterns.length} patterns`,
		);

		// Run explain
		log("  Running explain...");
		const explainResult: ExplainResult = await explainAnalysis(result, {
			apiKey,
			model,
		});
...
function compareFindingsStructured(
	...
	for (const [title, newF] of newByTitle) {
		...
		if (oldF.severity !== newF.severity)
			changes.push(`severity: ${oldF.severity} → ${newF.severity}`);
		if (oldF.confidence !== newF.confidence)
			changes.push(`confidence: ${oldF.confidence} → ${newF.confidence}`);
		if (oldF.category !== newF.category)
			changes.push(`category: ${oldF.category} → ${newF.category}`);
		if (oldF.description !== newF.description)
...
async function generateDiffReport(
	...
	const allProfileNames = [
		...new Set([...profileNames, ...prevProfileNames]),
	].sort();
	...
	await Promise.all(
		allProfileNames.map(async (name) => {
			...
			if (existsSync(curDir))
				currentData.set(name, await loadProfileData(curDir));
			if (existsSync(prevDir))
				previousData.set(name, await loadProfileData(prevDir));
		}),
	);
	...
	lines.push(
		`| Total cost | $${curTotalCost.toFixed(4)} | $${prevTotalCost.toFixed(4)} | ${fmtSign(costDelta, 4)} |`,
	);
	lines.push(
		`| Input tokens | ${curTotalIn} | ${prevTotalIn} | ${fmtSign(inDelta)} |`,
	);
	lines.push(
		`| Output tokens | ${curTotalOut} | ${prevTotalOut} | ${fmtSign(outDelta)} |`,
	);
	lines.push(
		`| Findings count | ${curTotalFindings} | ${prevTotalFindings} | ${fmtSign(findingsDelta)} |`,
	);
	lines.push("");
	...
	for (const name of allProfileNames) {
		...
		lines.push(
			`- Input tokens: ${fmtDelta(curCost.inputTokens, prevCost.inputTokens)}`,
		);
		lines.push(
			`- Output tokens: ${fmtDelta(curCost.outputTokens, prevCost.outputTokens)}`,
		);
		lines.push("");
...
async function runOnce(
	...
	const costs = await Promise.all(
		profiles.map((p) =>
			analyzeOne(p, runDir, apiKey, model, payloadConfig, promptConfig),
		),
	);
...
async function main(): Promise<void> {
	...
	if (configName !== undefined) {
		...
		if (!payloadConfig) {
			log(
				`ERROR: Unknown config "${configName}". Available: ${Object.keys(PAYLOAD_PRESETS).join(", ")}`,
			);
			process.exit(1);
		}
	}
	...
	if (promptName !== undefined) {
		...
		if (!promptConfig) {
			log(
				`ERROR: Unknown prompt "${promptName}". Available: ${Object.keys(PROMPT_PRESETS).join(", ")}`,
			);
			process.exit(1);
		}
	}
	...
	log(
		`Found ${profiles.length} profiles (model: ${model}, config: ${configLabel}, runs: ${numRuns})`,
	);
