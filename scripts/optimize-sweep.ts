...
async function saveProgress(progress: SweepProgress): Promise<void> {
	...
	await writeFile(
		resolve(RUNS_DIR, "sweep-progress.json"),
...
async function runCompareScript(
	aRuns: string[],
	bRuns: string[],
): Promise<string> {
	const cmd = [
...
async function generateResultsMatrix(progress: SweepProgress): Promise<string> {
	const lines: string[] = [];
	lines.push("\n## Phase 1 Results Matrix\n");
	lines.push(
		"| Config | Opus Runs | Sonnet Runs | vs Opus W/L/D | Verdict | Notes |",
	);
	lines.push(
		"|--------|-----------|-------------|---------------|---------|-------|",
	);
	...
	for (const [configName, runs] of Object.entries(progress.configRuns)) {
		...
		if (reportPath) {
			...
			if (scores) {
				...
				verdict =
					scores.overall.verdict === "B"
						? "Sonnet"
						: scores.overall.verdict === "A"
							? "Opus"
							: "Draw";
				// Average scores
				const avgA =
					scores.profiles.reduce((s, p) => s + p.groupA_avg, 0) /
					scores.profiles.length;
				const avgB =
					scores.profiles.reduce((s, p) => s + p.groupB_avg, 0) /
					scores.profiles.length;
		...
		lines.push(
			`| ${configName} | ${progress.opusRuns.length} | ${runs.length} | ${wld} | ${verdict} | ${notes} |`,
		);
	}
...
async function main(): Promise<void> {
	...
	for (const c of allConfigs) {
		if (!PAYLOAD_PRESETS[c]) {
			log(
				`ERROR: Unknown config "${c}". Available: ${Object.keys(PAYLOAD_PRESETS).join(", ")}`,
			);
			process.exit(1);
		}
	}
	...
	if (opusRunsArg) {
		...
		log(
			`Reusing ${progress.opusRuns.length} existing Opus runs: ${progress.opusRuns.join(", ")}`,
		);
	} else if (!skipOpus) {
		log("--- Phase 0: Opus baseline ---");
		const opusRuns = await runTestScript([
			"--model",
			"opus",
			"--runs",
			String(numRuns),
	...
	if (progress.opusRuns.length === 0) {
		log(
			"ERROR: No Opus baseline runs available. Run without --skip-opus first.",
		);
		process.exit(1);
	}
