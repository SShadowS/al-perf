...
2. **Hysteresis for auto-file** (spec: "fire only after M consecutive over-threshold runs"): a `create-issue` row is enqueued only when `autoFile` is true, the finding's severity ranks at/above `autoFileMinSeverity` (default `critical`), `countQualifyingOccurrences(findingId) >= autoFileAfterRuns` (default 2) — counting only **qualifying (non-incomplete) occurrences**, since the umbrella spec excludes incomplete captures from lifecycle run-counting and auto-file hysteresis IS run-counting; an incomplete capture must never push a finding over the threshold — the finding's `absenceCount` is 0 (currently present), its state is active (`new/open/regressed/improving`), and no issue mapping exists yet. The dedupe key `github:create:<tenant>:<fingerprint>` makes re-scans and duplicate enqueues structurally impossible (UNIQUE column).
