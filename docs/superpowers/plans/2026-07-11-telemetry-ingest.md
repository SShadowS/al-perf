...
- One `hotspots[]` entry per SIGNAL carrying the signal's REAL identity (functionName = methodName, objectType, objectId, appId, appName, zero times) — this both makes the batch's apps "exercised" (`exercisedAppsOf` dedupes appIds itself) and lets `collectFindings`' method-index lookup attach the correct app to each telemetry finding row, which app-scoped absence gating depends on. *(Amended during Task 3: the original "one placeholder hotspot per app" rule left telemetry finding rows with appId="", making absence accrue on every batch regardless of app — confirmed empirically.)*
