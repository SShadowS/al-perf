...
- **`buildTableRelationGraph` attributed every extension's relations to a table that does not exist.** It walked `TableExtension` objects and emitted the extension's own name as `fromTable`, so `tableextension "CDC Continia User Setup" extends "CTS-CBF Continia User Setup"` produced eight edges out of "CDC Continia User Setup". 190 edges on the 15,436-file corpus were renamed onto the base table they actually belong to, and the count of edges whose source table no root declares drops from 183 to 8; on Document Output it is 50 of 123 edges. The function is exported from the library API and read by no CLI, MCP or web output, so this is a fix for external callers only.
