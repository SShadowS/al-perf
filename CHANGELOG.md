...
- **`incomplete-setloadfields` no longer reports `critical`, because its claim was wrong.** It said a field left out of `SetLoadFields` "will return default values or cause runtime errors". Neither happens. The platform JIT-loads the field — an implicit `Get` against the data source — so the value is correct and the cost is an extra round-trip that partly undoes the narrowing (Microsoft Learn, *Using partial records*). Measured on a BC 28 container rather than inferred: 5,000 iterations reading an unloaded field cost **4** SQL statements against **1** for a covering load set — a small constant, not one round-trip per row. The lasting cost is that the field joins the load set, so the narrowing stops applying from that point on. Errors are possible only under a race, when another session modifies or deletes the row between the two loads. The detector is a performance finding now: a confirmed missing field is **warning** (was critical) and an unconfirmable name is **info** (was warning). On a 15,436-file corpus that moves 68 findings out of `critical` entirely, so `gate --max-critical 0` gets *easier*, not harder, for this detector.
...
- **The source index now models a table as its root declaration plus every indexed `tableextension`.** Detector counts move in both directions, and three separate causes are in play: the merge itself, a new refusal to answer for a table name that two different roots declare (16 of them with the base app in-tree — namespaces make same-name tables legal), and a latent case-sensitivity bug the new lowercased lookup fixes. A fourth, unrelated to tables, ships alongside them: `SetLoadFields` suggestions no longer name FlowFields. On a 15,436-file corpus with the base app in-tree, 150 findings move — 53,351 → 53,205 — all of them in `unindexed-filter` (9,098 → 8,967, entirely warnings, split 96 removed by the ambiguity fence / 36 by the merge / 1 by the FlowFilter guard / 2 added by the case fix) and `incomplete-setloadfields` (94 → 79, entirely the FlowField guard). On every corpus measured the only criticals that moved went **down**, 83 → 68. That is not a guarantee: a workspace that indexes a table *and* its extensions can in principle gain criticals, because a confirmed extension field is now rated critical where the old root-only lookup resolved nothing and hedged, an extension-contributed `Sum` can graduate a bare `CalcFields()` in a loop from warning to critical, and an ambiguous name falls back to the conservative critical. Budget a look at the diff rather than assuming the gate verdict is safe. Three partner apps move by zero *findings* each — every `tableextension` in them targets a base-app root that lives in `.dependencies/` and is correctly never indexed, and a table known only from its extensions still refuses every negative question — but their relation graphs are substantially recut: 50 of Document Output's 123 edges were filed under a `tableextension`'s own name and now name the base table (library API only — no CLI, MCP or web output reads that graph). `SourceIndex` gains a required `tables` field, which breaks any external caller that constructs one, and `CACHE_VERSION` moves 3 → 4, so the first run after upgrading re-parses.
...
- **Every savings estimate changed, and four of them are now measured rather than guessed.** `estimatedSavings` was the pattern's impact times a fixed fraction written from judgement. Four of those fractions were checked against a BC 28 container and all four were wrong, in both directions: `modify-in-loop` claimed 60% and buys **7%**, `dangerous-call-in-loop` claimed 90% and buys **59%**, `calcfields-in-loop` claimed 80% and buys **96%**, `record-op-in-loop` claimed 70% and buys **95%**. If you rank or budget work off `estimatedSavings`, the ordering moves. The six that remain unmeasured now say `Rough estimate (not measured)` in their text, so the two kinds of number are distinguishable for the first time.
...
- **`ModifyAll` and `DeleteAll` are not set-based on BC 28, so the detectors stop implying they are.** `modify-in-loop` and `delete-in-loop` both suggested them as the fix, and `what-if.ts` promised a ~60% saving from "batching modifications with ModifyAll". Measured: both issue one `UPDATE`/`DELETE` per row keyed on the primary key — visible in the captured SQL as `DELETE FROM … WHERE ("Entry No_"=@0)` with an execution count equal to the row count — so replacing a loop with them buys **6–7%**, not an order of magnitude (2005 statements vs 2004 at N=2000; 2584 ms vs 2409 ms at N=20000). Two confounders were ruled out rather than argued away: a control table with no secondary key measures identically, and the wall-clock run at 10× N rules out the platform batching those statements into fewer round-trips. Both suggestions now name the lever that does scale — touch fewer rows.
- **`record-op-in-loop` names the edit instead of a direction.** It said "restructure to reduce database calls inside the loop", which is not something you can apply, and which is why its savings figure had never been checkable. It now says to read the looked-up table once into a temporary record before the loop — measured at 2001 SQL statements and 167 ms becoming 2 and 9 ms over 2,000 rows — carrying the condition that makes it safe: only where the looked-up set fits in memory. Advising a preload for an unbounded table would trade a slow loop for an out-of-memory error.
...
- **Four consumers resolved a table by scanning every object, once per record variable.** `detectUnindexedFilters` looped `index.objects.values()`, `detectIncompleteSetLoadFields` spread the whole 15,411-entry map into a fresh array before `.find()`, and `resolveCalcFields` and `buildTableRelationGraph` each walked it too. All four now read one prebuilt map. Detector phase on the 15,436-file corpus, two serial runs per side: 20.9s / 17.6s → 5.7s / 5.9s. Building the map costs nothing measurable — indexing is 57.0s / 56.0s before and 56.8s / 56.4s after.
...
- **The web report showed no savings estimate at all, and no formatter showed the reasoning behind one.** Section-level formatter parity is enforced at compile time; nothing enforced it per FIELD, so a field could be carried on every finding, populated on every analysis, and rendered by nobody. `savingsExplanation` was exactly that — it reached users only through raw JSON. Five fields were missing somewhere: `savingsExplanation`, `evidence` (the finding's line and column) and `involvedMethods` (its object type and id) from all three human formatters, `estimatedSavings` and the pattern `id` from HTML. All now render everywhere, and a test checks parity two ways — statically per field, and by pushing sentinel values through each formatter, because a static reference check cannot tell an emit from a mention in a condition and was in fact satisfied by a same-named field on a different type.
- **A finding with no measured time still reported a savings percentage of it.** `dangerous-call-in-loop` and `external-call-in-loop` fire without a profile and carry `impact: 0` by construction, and any fraction of zero is zero — so a finding rated critical shipped the sentence "removes ~59% of the 0µs", which reads as "fixing this is worth nothing". No measured time now means no estimate; the finding keeps its `suggestion`, which is where its advice already lived.
...
- **`unfiltered-findset` reported in-memory records as full table scans.** It built its own temporary-record gate from `var` declarations, while every record-op detector used the shared `isTemporaryOp`, which also reads the owning object's `SourceTableTemporary`. A Page or Query with that set has an in-memory `Rec` and no `var` declaration anywhere for a local scan to find, so every unfiltered find on one was reported as reading the whole table — 89 findings on a 15,436-file corpus, 126 on a 19,141-file one, all describing SQL that never runs. Both now use the one gate.
- **A Record built-in was reported as a field missing from `SetLoadFields`.** `Rec.ReadPermission` with no parentheses parses exactly like `Rec."Document No."`, so it was recorded as a field access. Several of these names were already listed as non-fields for escape analysis; a single shared `PARENLESS_RECORD_BUILTINS` now feeds both. `Number` and `RecordID` are deliberately excluded, since real tables declare fields by those names.

- **FlowFields whose `CalcFormula` the extractor could not type were treated as ordinary fields.** 130 of 2,625 FlowFields on the corpus carried `FieldClass = FlowField` with no resolved formula type, because two ordinary AL shapes never produced the node the extractor looked for: an aggregate with no `where` clause (`CalcFormula = count("VAT Group Approved Member")`) parses as a plain call expression, and a negated aggregate (`CalcFormula = - sum(...)`, how every "balance owed" FlowField in BC is written) leaves the property with no `value` field at all. Seven `incomplete-setloadfields` findings were suggesting `SetLoadFields("reserved quantity")` — a FlowField, which does not compile. Unresolved FlowFields drop 130 → 4, those findings drop to 0, and the guard now also checks `FieldClass` rather than trusting the extractor to be complete. `calcfields-in-loop` severity does not move: the newly typed fields are all aggregates, which already took the conservative `critical` default — but the rating is now evidence-based, and the suggestion's factual clause can speak where it previously stayed silent.
- **`calcfields-in-loop` claimed things about the table that were true only of one field.** The suggestion said "This table has Lookup FlowFields" whenever the calculated field was a Lookup — false as soon as the table also carries a `Sum` nobody is calculating here. The severity beside it was already field-scoped; the prose now agrees, reading "The field(s) calculated here are …".
- **`incomplete-setloadfields` hedged about fields it had confirmed.** When one `SetLoadFields` covered a confirmed extension field and an unconfirmable name together, the text said "these names could not be confirmed to be fields at all" about both. It now names only the subset actually in doubt.
- **`unindexed-filter` evidence rendered the same key name twice with contradictory field lists.** An extension may legally reuse a base key's name, so the key list could read `ByDate(Posting Date), ByDate(Ext Code)` with nothing to explain it. Keys contributed by an extension are now tagged `[from extension <id>]`.
...
- **A table is not one object, and every detector was reading a fragment of it.** A BC table is a root `table` declaration plus any number of `tableextension` objects from any number of apps, and four consumers each re-derived it by scanning `index.objects` for the first object whose name matched. `unindexed-filter` could not see a key an extension declared — `key(Key8; "Production BOM No.")` on Item lives in `MfgItem.TableExt.al`, and code filtering on it even calls `SetCurrentKey` first — nor a FlowFilter field an extension adds; `incomplete-setloadfields` could not confirm an extension field is a field; and `calcfields-in-loop` could not resolve an extension FlowField's CalcFormula, so it fell back to a conservative `critical` every time. Measured on the 15,436-file corpus: 144 findings move, 142 out and 2 in, and 376 relation edges move. 37 of the removed findings trace to something an extension declares — 27 filters on an extension-declared key, 1 on an extension-declared FlowFilter, and 9 more suppressed as their siblings once one filter in the member finally hit a key.
- **A table name that two different roots declare was answered with whichever one the scan reached first.** AL has namespaces, so `table 480 "Dimension Set Entry"` in the base app and `table 36950 "Dimension Set Entry"` in PowerBI Reports are both legal and both real. The linear scan returned the PowerBI table, whose two keys are `PK(Dimension Set ID)` and `SystemModifiedAtKey`; every `unindexed-filter` judgement about the base-app table was therefore made against the wrong key list, and the base table's own `key(Key2; "Dimension Value ID")` was invisible. 16 names are ambiguous with the base app in-tree, and an ambiguous entry now answers nothing at all: 96 `unindexed-filter` findings and 375 relation edges withdrawn, none of them supportable.
- **Table lookups compared names case-sensitively, and AL identifiers are case-insensitive.** `QuoteSalesHeader: Record "Sales header"` compiles and is a `Sales Header`, but `objectName === variable.tableName` matched nothing, so every detector that needed the table silently skipped the variable. 46 record declarations on the corpus are spelled this way — `"Sales header"`, `"Unit Of Measure"`, `"g/l Entry"`, `"Vat Posting Setup"` — and the new lowercased map resolves them. Two real `unindexed-filter` findings appear as a result; nothing about extensions is involved.
- **`buildTableRelationGraph` attributed every extension's relations to a table that does not exist.** It walked `TableExtension` objects and emitted the extension's own name as `fromTable`, so a `tableextension` extending a base table produced its edges out of the extension's own name — a table that does not exist. 190 edges on the 15,436-file corpus were renamed onto the base table they actually belong to, and the count of edges whose source table no root declares drops from 183 to 8; on Document Output it is 50 of 123 edges. The function is exported from the library API and read by no CLI, MCP or web output, so this is a fix for external callers only.
- **`incomplete-setloadfields` suggested adding FlowFields to a `SetLoadFields` list.** `SetLoadFields` does not accept FlowFields or FlowFilters, so the suggested code does not compile. All nine of the detector's removed criticals on the corpus are this: `CustLedgerEntry.CalcFields(Amount, "Amount (LCY)")` followed by reading `CustLedgerEntry.Amount` was reported `critical`, "these fields will return default values or cause runtime errors", about code that had already done the right thing.
